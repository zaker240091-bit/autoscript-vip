...
if [[ "${ID}" == "ubuntu" && ("${VERSION_ID}" == "22.04" || "${VERSION_ID}" == "24.04") ]]; then
    ...
    echo -e "${ERROR} OS tidak didukung. Script ini disiapkan untuk Ubuntu 22.04/24.04 dan Debian 12. Terdeteksi: ${PRETTY_NAME:-unknown}"
    exit 1
fi
...
function first_setup(){
    ...
    apt-get update -y
    # On Debian 12, add backports and skip Ubuntu-only packages
    if [[ "${ID}" == "debian" && "${VERSION_ID}" == "12" ]]; then
        if ! grep -q "bookworm-backports" /etc/apt/sources.list /etc/apt/sources.list.d/*.list 2>/dev/null; then
            echo "deb http://deb.debian.org/debian bookworm-backports main" > /etc/apt/sources.list.d/backports.list
        fi
        apt-get update -y
        apt-get install "${APT_ARGS[@]}" --no-install-recommends ca-certificates curl wget gnupg lsb-release debconf-utils apt-transport-https
    else
...
function base_package() {
    ...
    # Install packages with OS-specific adjustments
    COMMON_PKGS="vnstat libnss3-dev libnspr4-dev pkg-config libpam0g-dev libcap-ng-dev libcap-ng-utils libselinux1-dev libcurl4-nss-dev flex bison make libnss3-tools libevent-dev bc rsyslog dos2unix zlib1g-dev libssl-dev libsqlite3-dev sed dirmngr libxml-parser-perl build-essential gcc g++ python3 python3-pip htop lsof tar wget curl ruby zip unzip p7zip-full libc6 util-linux msmtp-mta ca-certificates bsd-mailx iptables iptables-persistent netfilter-persistent net-tools openssl gnupg gnupg2 lsb-release cmake git screen socat xz-utils apt-transport-https dnsutils cron bash-completion ntpdate chrony jq openvpn easy-rsa"
    if [[ "${ID}" == "debian" && "${VERSION_ID}" == "12" ]]; then
        # python-is-python3 is in Debian 12 main repos
        apt-get install "${APT_ARGS[@]}" $COMMON_PKGS python-is-python3 || true
    else
        apt-get install "${APT_ARGS[@]}" $COMMON_PKGS python-is-python3 || true
    fi
...
# Restart critical services on boot (Debian 12 compatibility)
sleep 5
systemctl restart nginx >/dev/null 2>&1 || true
systemctl restart haproxy >/dev/null 2>&1 || true
systemctl restart noobzvpns >/dev/null 2>&1 || true
...
rm -f noobzvpns.zip install.sh LICENCE.MD README.MD badge.zip cert.pem key.pem config.json noobzvpns.x86_64 uninstall.sh

# Fix noobzvpns service to start AFTER nginx — prevents port 80 conflict on reboot
if [ -f /etc/systemd/system/noobzvpns.service ]; then
    sed -i 's/After=network.target network-online.target$/After=network.target network-online.target nginx.service/' /etc/systemd/system/noobzvpns.service
    sed -i 's/After=network.target$/After=network.target network-online.target nginx.service/' /etc/systemd/system/noobzvpns.service
    systemctl daemon-reload
fi

# Install cekconfig tool for viewing saved configs
mkdir -p /root/configs
cat > /usr/local/sbin/cekconfig << 'CKCEOF'
#!/bin/bash
clear
echo -e "\033[0;34m◇━━━━━━━━━━━━━━━━━━━━━━━━━━━◇\033[0m"
echo -e "       SAVED CONFIGS VIEWER       "
echo -e "\033[0;34m◇━━━━━━━━━━━━━━━━━━━━━━━━━━━◇\033[0m"
echo ""
if [ ! -d /root/configs ] || [ -z "$(ls /root/configs 2>/dev/null)" ]; then
    echo -e "\033[0;31mNo saved configs found in /root/configs/\033[0m"
    echo ""
    read -n 1 -s -r -p "Press any key to back on menu"
    menu; exit 0
fi
echo -e "\033[0;32mSaved configs:\033[0m"
echo ""
ls /root/configs/ | nl
echo ""
read -p "Select number to view (0 to back): " num
if [[ $num == "0" ]]; then menu; exit 0; fi
file=$(ls /root/configs/ | sed -n "${num}p")
if [ -z "$file" ]; then echo "Invalid."; sleep 1; cekconfig; fi
clear
echo -e "\033[0;34m◇━━━━━━━━━━━━━━━━━━━━━━━━━━━◇\033[0m"
cat /root/configs/$file
echo -e "\033[0;34m◇━━━━━━━━━━━━━━━━━━━━━━━━━━━◇\033[0m"
echo ""
read -n 1 -s -r -p "Press any key to back"
menu
CKCEOF
chmod +x /usr/local/sbin/cekconfig

systemctl restart noobzvpns || true

# Debian 12: install libssl1.1 required by noobzvpns binary
if [[ "${ID}" == "debian" && "${VERSION_ID}" == "12" ]]; then
    if ! dpkg -l 2>/dev/null | grep -q libssl1.1; then
        wget -q -O /tmp/libssl1.1.deb "http://security.ubuntu.com/ubuntu/pool/main/o/openssl/libssl1.1_1.1.1f-1ubuntu2_amd64.deb"
        dpkg -i /tmp/libssl1.1.deb >/dev/null 2>&1 || true
        rm -f /tmp/libssl1.1.deb
    fi
    # Create proper noobzvpns service for Debian 12
    if [ ! -f /etc/systemd/system/noobzvpns.service ] || ! systemctl is-active noobzvpns >/dev/null 2>&1; then
        cat > /etc/systemd/system/noobzvpns.service << SVCEOF
[Unit]
...
[Service]
Type=simple
ExecStart=/usr/local/bin/noobzvpns server --domain ${domain} --cert /etc/xray/xray.crt --key /etc/xray/xray.key
Restart=always
RestartSec=3
...
SVCEOF
        systemctl daemon-reload
        systemctl enable noobzvpns >/dev/null 2>&1 || true
        systemctl restart noobzvpns || true
    fi
fi
...
/etc/init.d/cron restart || systemctl restart cron || true
    ...
    systemctl enable --now noobzvpns >/dev/null 2>&1 || true
history -c
...
print_install "Enable Service"
    ...
    systemctl enable --now openvpn-server@server-tcp openvpn-server@server-udp >/dev/null 2>&1 || true
    # Start nginx FIRST then haproxy then noobzvpns — prevents port 80 conflict
    systemctl restart nginx >/dev/null 2>&1 || true
    sleep 2
    systemctl restart haproxy >/dev/null 2>&1 || true
    sleep 1
    ...
    # Add crontab reboot fix for service order
    (crontab -l 2>/dev/null | grep -v "restart nginx haproxy noobzvpns"; echo "@reboot sleep 15 && systemctl restart nginx && sleep 2 && systemctl restart haproxy && sleep 1 && systemctl restart noobzvpns") | crontab -
    # Create /root/configs directory for saved account configs
    mkdir -p /root/configs
